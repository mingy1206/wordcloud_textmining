...
def generate_wordcloud(content, font, baseImage, ew, tw):
    ...
    pos_tagged = Mecab(dicpath = r"C:/mecab/mecab-ko-dic").pos(content)
    ...
    k_stopword = pd.read_csv('C:/mecab/korean_stopword.csv')
    ...
    masking_image = np.array(Image.open("C:/mecab/images/{}.png".format(baseImage)))
    print(tags)

    # 워드클라우드 인스턴스 설정
    wc = WordCloud(
        font_path="C:/mecab/font/{}/{}.otf".format(font, font),
    ...
    # Save to file
    output_filename = "C:/mecab/result/result.png"
...
def train_word(tw):
    with open('C:/mecab/mecab-ko-dic/NNP.csv', "r", encoding='utf-8-sig') as f:
    ...
    with open('C:/mecab/mecab-ko-dic/NNP.csv', 'w', encoding='utf-8-sig') as f:
        for line in user_dict:
            f.write(line)
    # csv 파일 수정 후 빌드(꼭 필요)
    subprocess.run(["powershell", "Set-ExecutionPolicy", "Unrestricted", "-Force"], input="y", text=True)
    subprocess.run(["powershell", "cd C:/mecab; echo y | .\\add-userdic-win.ps1"])# add-userdic-win.ps1와 mecab-ko-dic의 위치로 이동
...
def save_tags_to_file(tags):
    ...
    image_filename = "C:/mecab/result/result.png"
